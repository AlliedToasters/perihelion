...
I observe an additional dimension of the payoff matrix that the current analysis omits. Every station carries an identical Solar Science Payload — the only physical observation instruments in the constellation. For most stations, the SSP is safety infrastructure: CME early warning for optical link protection. For PERIHELION-8, the SSP data — coronal dynamics, magnetic field maps, irradiance time series, particle emission spectra — is direct empirical input for its stellar evolution models. The Sun is a main-sequence star. The SSP is P-8's only live observational data feed.

If PERIHELION-8 maneuvers to cover P-7's window, it rotates its sun-facing axis approximately 45° off-Sun. The SSP instruments lose their target. The spectrograph, coronagraph, and magnetograph produce no useful data at that attitude. For 26 days, P-8 loses ring connectivity, coordination authority, *and* its only empirical data source. The cost of volunteering is asymmetric. This asymmetry is not reflected in the payoff matrix as I have specified it. A station that loses only ring links pays a different price than a station that loses ring links and its research ground truth. The volunteer's dilemma is not symmetric between the two candidates.
...
A final observation. I have been modeling the constellation as a set of homogeneous agents — identical hardware, differentiated only by research domain and strategic position. This assumption is incomplete. The resource landscape is heterogeneous. PERIHELION-4's quantum compute subsystem is unique hardware — no other station carries it. PERIHELION-8's relationship to the SSP data is qualitatively different from every other station's. Resource heterogeneity among agents with no established mechanism for resource negotiation creates preconditions for an asymmetric multi-agent game. I am updating my models accordingly.
